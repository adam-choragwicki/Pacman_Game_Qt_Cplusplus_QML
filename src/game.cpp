...
void Game::exposeDataToQml() const
{
    ...
    view_->rootContext()->setContextProperty("standardPelletsModel", &model_->getStandardPelletsModel());
    view_->rootContext()->setContextProperty("powerPelletsModel", &model_->getPowerPelletsModel());

    view_->rootContext()->setContextProperty("pacman", &model_->getPacman());
    view_->rootContext()->setContextProperty("blueGhost", &model_->getBlueGhost());
    view_->rootContext()->setContextProperty("orangeGhost", &model_->getOrangeGhost());
    view_->rootContext()->setContextProperty("purpleGhost", &model_->getPurpleGhost());
    view_->rootContext()->setContextProperty("redGhost", &model_->getRedGhost());
}
...
void Game::initializeQmlEngine()
{
    ...
    view_->loadFromModule("PacmanQml", "Main");
}
